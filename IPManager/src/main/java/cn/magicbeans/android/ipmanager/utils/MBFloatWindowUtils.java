...
public class MBFloatWindowUtils {

    public void init(final Activity activity, String ip, String port, String name) {
        if (MBIPUtils.getInstance(activity).getIPPort() == null) {
            MBIPInfo info = new MBIPInfo(ip, port, name, 1);
            MBIPUtils.getInstance(activity).insertIPPort(info);
        }
        final View floatView = LayoutInflater.from(activity).inflate(R.layout.flaot_mb_layout, null);
